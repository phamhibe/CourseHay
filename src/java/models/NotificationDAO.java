...
public class NotificationDAO extends DBContext {
    ...
    public ArrayList<Notification> getListNotiByAccountId(int accountid, int sendto) {
        ...
        try {
            String sql = "select AccountId,NotifyContent,SendTo,Status,StatuTime,CourseId from notification where AccountId =? and Sendto = ? order by StatuTime desc;";
            PreparedStatement ps = connection.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                notification.add(new Notification(AccountId, NotifyContent, SendTo, Status, StatuTime, CourseId));
            }
        } catch (SQLException e) {
        }
        return notification;
    }
    public void HasSeenNoti(int courseid){
        
        try {
            String sql = "update notification set Status = ?  where courseid = ? ;";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setInt(1, 1);
    ...
    public static void main(String[] args) {
        ArrayList<Notification> n = new NotificationDAO().getListNotiByAccountId(8, 1);
