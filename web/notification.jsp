                        ...
                        <c:forEach items="${listnotification}" var="n">
                            <div class="product" style="border: 1px solid saddlebrown; margin: 0px 10px; background-color:
                                ...
                                <c:if test="${n.getSendTo() == 1}">
                                    <div style="margin: 1% 10px"><a href="<%=path%>/notifications?role=instructor&CourseId=${n.getCourseId()}">Notification</a></div>
                                </c:if>
                                <c:if test="${n.getSendTo() == 0}">
                                    <div style="margin: 1% 10px"><a href="<%=path%>/notifications?role=student&CourseId=${n.getCourseId()}">Notification</a></div>
                                </c:if>
